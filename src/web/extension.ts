...
export function activate(context: vscode.ExtensionContext) {
	...
	const disposable = vscode.commands.registerCommand('reelrender.preview', () => {
		...
		const customCss = [
			getCss(context, panel, 'prism-vsc-dark-plus.css'),
		];

		// Initial HTML
		panel.webview.html = getWebviewContent(customCss);
		...
		vscode.workspace.onDidChangeTextDocument(event => {
			if (event.document === vscode.window.activeTextEditor?.document) {
				panel.webview.postMessage(event.document.getText());
			}
		});
...
function getWebviewContent(cssUris?: vscode.Uri[]): string {
	const css = cssUris
...
function getCss(
	...
	const onDiskPath = vscode.Uri.joinPath(context.extensionUri, 'media', cssFile);
